    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write("This is a placeholder text for vector embeddings.")
        ...
        if not content.strip():
            print("Warning: File is empty. Adding placeholder content.")
            content = "This is a placeholder text for vector embeddings."
            # Save the placeholder content to the file
        ...
        # Create a fallback solution with minimal content
        try:
            print("Attempting fallback with minimal content...")
            
            # Clear resources even in fallback mode
            global vectors, retrieval_chain, memory
            
            # Clear Chroma DB if it exists
            if 'vectors' in globals() and vectors is not None:
                try:
                    print("Clearing existing vector store in fallback...")
                    vectors.delete_collection()
                    vectors = None
                except Exception as clear_error:
                    print(f"Error clearing vector store in fallback: {str(clear_error)}")
            
            # Clear retrieval chain
            if 'retrieval_chain' in globals() and retrieval_chain is not None:
                print("Clearing existing retrieval chain in fallback...")
                retrieval_chain = None
            
            # Clear memory
            if 'memory' in globals() and memory is not None:
                print("Clearing existing memory in fallback...")
                memory = None
            
            # Create minimal content for fallback
            minimal_content = "This is a fallback text for vector embeddings."
            minimal_docs = [Document(page_content=minimal_content)]
            
            print("Initializing new vector store in fallback...")
            vectors = Chroma.from_documents(minimal_docs, embeddings)
            
            print("Creating new retrieval chain in fallback...")
            rag_prompt = ChatPromptTemplate.from_template(
                """
            ...
            print("Initializing fresh memory in fallback...")
            memory = ConversationSummaryBufferMemory(
                llm=llm,
                max_token_limit=2000,
                return_messages=True
            ...
            print("Fallback context loaded successfully!")
            return vectors, retrieval_chain, memory
